...
references:: Specifies other TypeScript projects that this configuration depends on, typically used to organize code into smaller, more manageable pieces.
+
Setting this to *[{ "path": "./src" }]* tells TypeScript to look for additional `tsconfig.json` files in your source directory, allowing you to split your Foundry module's configuration into separate components.
====
