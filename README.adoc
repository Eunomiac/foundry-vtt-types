...
forceConsistentCasingInFileNames:: Ensures that casing in imported files must exactly match the actual filename. For example, `import "./MyFile"` won't work if the file is actually named `myfile.ts`.
+
Setting this to *true* prevents bugs that could occur when your project runs on case-sensitive systems. _(This is especially important if anyone contributing to your project develops on Windows where imports _are_ case insensitive because your users might be running Foundry on Linux, where `MyFile.ts` and `myfile.ts` are treated as different files.)_

resolveJsonModule:: Allows importing JSON files directly for automatic types.
+
...
noUncheckedIndexedAccess:: Adds `undefined` to types with index signatures. Index signatures are like catch-alls for particular shapes of keys. For example an array in TypeScript will allow expressions like `arr[10]` because that key may exist at runtime. However with `noUncheckedIndexedAccess` set to false (the default) the resulting types will not include `undefined` even though at runtime such a key may not exist.
+
...
noUncheckedSideEffectImports:: Ensures TypeScript does not silently allow invalid import paths when no values or types are imported from it. For example `import "./foo/bar";` would not error if that path does not really exist but `import { foo } from "./foo/bar";` would.
+
...
+
Setting this to *true* ensures safer error handling by preventing you from accidentally misusing the error, such as assuming a certain property exists. The type cannot be more specific because in JavaScript it's possible to throw any value, e.g. `throw "foo bar";` and `throw 1;`. To check if it's an `Error` you will have to check with `if (err instanceof Error)`.
